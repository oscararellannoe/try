...
export default function WhyMega() {
  return (
    <section id="porque" className="relative px-4 py-24 sm:px-10">
      <div className="mx-auto max-w-6xl rounded-[40px] border border-white/15 bg-white/5 p-10 text-center shadow-neon backdrop-blur-2xl">
        <p className="text-sm uppercase tracking-[0.5em] text-white/60">¿Por qué Mega Mangos?</p>
        <h2 className="mt-4 text-4xl font-black text-white sm:text-5xl">
          Sabemos a nostalgia, fiesta y familia.
        </h2>
        <p className="mx-auto mt-3 max-w-3xl text-lg text-white/80">
        ...
        <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-3">
          {reasons.map((reason, index) => (
            <motion.div
              ...
              className="rounded-[28px] border border-white/10 bg-night/70 p-6"
            >
              <div className="text-4xl">{reason.icon}</div>
              <h3 className="mt-4 text-2xl font-black text-white">{reason.title}</h3>
              <p className="mt-2 text-white/80">{reason.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
